...
RUN set +x && apt update && apt -y upgrade && apt install sqlite3 festival speech-dispatcher espeak libsox-fmt-all sox git git-lfs net-tools bind9-utils netcat-traditional && apt clean && rm -rf /var/lib/apt/lists
